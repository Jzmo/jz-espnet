...
class DynamicConvolution(nn.Module):
    ...
    def __init__(
        ...
        #               \        /
        #                 Linear
        self.linear1 = nn.Linear(in_feat, out_feat * 2)
        self.linear2 = nn.Linear(out_feat, out_feat)
        self.linear_weight = nn.Linear(out_feat, self.wshare * 1 * self.kernel_size)
        ...
        if self.use_bias:
            self.bias = nn.Parameter(torch.Tensor(in_feat))
