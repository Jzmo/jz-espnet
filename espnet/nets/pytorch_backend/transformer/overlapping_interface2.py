...
def get_token_level_ids_probs(ctc_ids, ctc_probs):
    ...
    bh = len(ctc_ids[0]) // 2
    y_hat = torch.tensor([x[0].item() for x in groupby(ctc_ids[0])])
    y_idx = torch.nonzero(y_hat != 0).squeeze(-1)
    y_hat = y_hat[y_idx].tolist()
...
def dynamic_matching_xl2(t1, t2, prob1, prob2,
                         dp_prev=None, t1_prev=None, t2_prev=None,
                         reserved_t=None, reserved_p=None):

    if dp_prev is None:
        # , reserved_t, reserved_p)
        return dynamic_matching(t1, t2, prob1, prob2, reserved_t, reserved_p)
    # pdb.set_trace()
    if reserved_t == None:
        reserved_t = ([], [])
        reserved_p = ([], [])
