...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, n_head, n_feat, dropout_rate):
        ...
        self.rD = 3
        self.A = nn.Parameter(torch.Tensor(self.rD, self.n_feat).uniform_(0, 1))
    ...
    def forward(self, query, key, value, mask, R=None):
        ...
        if R is not None:
            ...
            a_expand = torch.zeros(B * T * T * self.n_feat * self.rD, device=query.device)
            a_expand = a_expand.view(B, T, T, self.rD, self.n_feat).fill_(float(0.0)).copy_(self.A)
            R = R.view(-1)
            a_expand = a_expand.view(-1, self.rD, self.n_feat)[torch.arange(B * T * T), R, :]
            a_expand = a_expand.view(B, T, T, H, -1).transpose(-2, -4).contiguous().transpose(-1, -2) # B x H x T x d_k x T
...
class RelBlockMultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask, key_memory=None, key_memory_mask=None, block_len=None, kmem_after=False):
        ...
        # need to change 0.1 to args.positional_dropout_rate
