    ...
    def recognize(self, x, recog_args, char_list=None, rnnlm=None):
        ...
        if decode_mode == "online":
            ...
            for t in range(x.size(0)):
                ...
                if (t+1) % (block_len * subsample) == 0 or (t == x.size(0) - 1 and t0//subsample < t//subsample):
                    logging.info("chunking feat {} {} {}".format(
                        t1, t+1, x[t1:t+1, :].size()))
                    if t1 > block_len * chuck_len * subsample:
                        # after first block
                        h_pad[:, t1//subsample:t//subsample, :] = self.encoder(
                            x[t1-block_len*chuck_len*subsample:t+1, :].unsqueeze(0), None, None, None)[0][:, block_len*chuck_len:, :]
                    else:
                        h_pad[:, t1//subsample:t//subsample, :] = self.encoder(
                            x[:t+1, :].unsqueeze(0), None, None, None)[0][:, t1//subsample:]
                    # greedy ctc output
                    ...
                    mask_num_concat = 0
                    if (t+1) % (block_len * 4 * subsample) == 0:
                        hist_length = sum(
                            y_in_length_cache[-cache_len:])
                        mask_hist = mask_cache[-hist_length:]
                    ...
                    if not mask_num_concat == 0 and hist_length > 0 and recog_args.maskctc_n_iterations > 0:

                        K = recog_args.maskctc_n_iterations

                        y_in_hist = y_in_cache[:, -hist_length:].clone()
                        ...
                        if False:
                            h_pad_in = h_pad[:, t0//subsample -
                                             cache_len*block_len*chuck_len:t//subsample, :]
                            hist_pred, _ = self.decoder(
                                torch.cat([y_in_hist, y_in], dim=-1), None, h_pad_in, None)
                            if hist_pred.size(1) > 0:
                                hist_pred_score, hist_pred_id = hist_pred[0].max(
                                    dim=-1)
                            for idx in range(y_in_hist.size(1)):
                                cached_idx = idx - y_in_hist.size(1)
                                if pred_score_cache[cached_idx] < hist_pred_score[idx] and mask_cache[cached_idx]:
                                    y_in_cache[0][cached_idx] = hist_pred_id[idx]

                        ret[-hist_length:] = y_in_concat.tolist()[0]
                        # pdb.set_trace()
                    ...
                    t0 = t+1
                    if (t+1) % (block_len * chuck_len * subsample) == 0:
                        t1 = t+1

                    # pdb.set_trace()
                if t == x.size(0) - 1:
                    break
                #ret = y_in_cache[0].tolist()
