    ...
    def recognize(self, x, recog_args, char_list=None, rnnlm=None):
        ...
        if decode_mode == "online":
            ...
            for t in range(x.size(0)):
                ...
                if (t + 1) % (block_len * subsample) == 0 or (
                    t == x.size(0) - 1 and t1 // subsample < t // subsample
                ):
                    logging.info(
                    ...
                    if t1 > block_len * subsample * chuck_len:
                        ...
                        h_pad[:, t1 // subsample : t // subsample, :] = self.encoder(
                            x[
                                t1 - block_len * subsample * chuck_len : t + 1, :
                            ].unsqueeze(0),
                            None,
                            None,
                            None,
                        ...
                        if t + bh * subsample < x.size(0):
                            h_pad_even[
                                :, t1 // subsample + bh : t // subsample + bh, :
                            ] = self.encoder(
                                x[
                                    t1
                        ...
                        h_pad[:, t1 // subsample : t // subsample, :] = self.encoder(
                            x[t1 : t + 1, :].unsqueeze(0), None, None, None
                        )[0]
                        if t + bh * subsample < x.size(0):
                            h_pad_even[
                                :, t1 // subsample + bh : t // subsample + bh, :
                    ...
                    if t + bh * subsample < x.size(0):
                        ctc_probs_even, ctc_ids_even = torch.exp(
                            self.ctc.log_softmax(
                                h_pad_even[:, t0 // subsample : t // subsample, :]
                            )
                        ).max(dim=-1)
                    ...
                    if (
                        ...
                        h_pad_in = h_pad[
                            :,
                            t1 // subsample - block_len * cache_len : t // subsample,
                            :,
                        ]
