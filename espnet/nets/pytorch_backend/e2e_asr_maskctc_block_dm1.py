...
class E2E(E2ETransformer):
    ...
    def recognize(self, x, recog_args, char_list=None, rnnlm=None):
        ...
        if decode_mode == "online":
            ...
            h_pad_even = torch.zeros(x.size(0)//subsample,
                                     self.adim, dtype=x.dtype).unsqueeze(0)
            logging.info(
            ...
            for t in range(x.size(0)):
                ...
                if (t+1) % (block_len * subsample) == 0 or (t == x.size(0) - 1 and t1//subsample < t//subsample):
                    ...
                    if t1 > block_len * subsample * chuck_len:
                        ...
                        if t + bh*subsample < x.size(0):
                            h_pad_even[:, t1//subsample + bh:t//subsample + bh, :] = self.encoder(
                                x[t1-block_len*subsample*chuck_len+bh*subsample:t+1+bh*subsample, :].unsqueeze(0), None, None, None)[0][:, block_len*chuck_len:, :]
                    else:
                        h_pad[:, t1//subsample:t//subsample, :] = self.encoder(
                            x[t1:t+1, :].unsqueeze(0), None, None, None)[0]
                        if t + bh*subsample < x.size(0):
                            h_pad_even[:, t1//subsample + bh:t//subsample + bh, :] = self.encoder(
                                x[t1:t+1, :].unsqueeze(0), None, None, None)[0]
                    # greedy ctc output
                    ...
                    if t + bh*subsample < x.size(0):
                        ctc_probs_even, ctc_ids_even = torch.exp(self.ctc.log_softmax(
                            h_pad_even[:, t0//subsample:t//subsample, :])).max(dim=-1)
                        _, pairs, probs = dynamic_matching(
                            ctc_ids[0], ctc_ids_even[0], ctc_probs[0], ctc_probs_even[0])
                        ctc_ids = tie_breaking(pairs)
                        ctc_probs = tie_breaking(probs)
                    #ctc_ids = torch.tensor([path])
                    # if sum(sum(ctc_ids)) > 0:
                    #    logging.info(
                    #       n2s(torch.stack([x[0] for x in groupby(ctc_ids[0])])))
                    #    pdb.set_trace()
                    ...
                    mask_num_concat = 0
                    iter_decode = False
                    if (t+1) % (block_len * subsample * chuck_len) == 0:
                        iter_decode = True
                        if cache_len > 0:
                            hist_length = sum(
