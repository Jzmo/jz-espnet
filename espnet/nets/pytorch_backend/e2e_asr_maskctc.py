            ...
            for t in range(x.size(0)):
                ...
                if (t + 1) % (block_len * subsample // 2) == 0 or (
                    ...
                    if t1 >= block_len * subsample * (chuck_len + 1 / 2):
                        ...
                        hs = self.encoder(x[: t + 1, :].unsqueeze(0), None)[0][
                            :,
                    ...
                    if ctc_ids_curr.size(1) % bh > 0:
                        ...
                        ctc_ids_curr = ctc_ids_curr[:, : -(ctc_ids_curr.size(1) % bh)]
                    ...
                    if (
                        ...
                        if t1 >= bh * subsample * (3 + cache_len):
                            h_pad_in = self.encoder(
                                x[
                                    t1 - bh * subsample * (3 + cache_len) : t1, :
                                ].unsqueeze(0),
                                None,
                            )[0][:, block_len:, :]
                        else:
                            h_pad_in = self.encoder(x[:t1, :].unsqueeze(0), None)[0]
