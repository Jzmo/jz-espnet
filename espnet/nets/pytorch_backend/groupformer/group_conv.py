...
class GroupLightConvolution(nn.Module):
    ...
    def __init__(
        ...
        super(GroupLightConvolution, self).__init__()
        ...
        self.act = nn.GLU()
        self.channel_map = nn.Parameter(torch.Tensor(self.wshare, self.wshare))
        nn.init.normal_(self.channel_map)
        # lightconv related
        self.weight = nn.Parameter(
            torch.Tensor(self.wshare, self.kernel_size).uniform_(0, 1)
        )
    ...
    def forward(self, query, key, value, mask):
        ...
        weight = torch.mm(F.softmax(self.channel_map, dim=-1), weight).unsqueeze(1)
