...
concatjson.py ${dir}/data_small.*.json > ${dir}/data_small.json

if [ $num_spkrs -eq 1 ]; then
  json2trn.py ${dir}/data_small.json ${dic} --num-spkrs ${num_spkrs} --refs ${dir}/ref.trn --hyps ${dir}/hyp.trn
  ...
  json2trn.py ${dir}/data_small.json ${dic} --num-spkrs ${num_spkrs} --refs ${ref_trns} --hyps ${hyp_trns}
